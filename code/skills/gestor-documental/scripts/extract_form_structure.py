"""
Extract form structure from a non-fillable Gestor Documental.

This script analyzes the Gestor Documental to find:
...
Usage: python extract_form_structure.py <input.Gestor Documental> <output.json>
"""
...
def extract_form_structure(Gestor Documental_path):
    structure = {
        "pages": [],
        "labels": [],
        "lines": [],
    ...
    with Gestor Documentalplumber.open(Gestor Documental_path) as Gestor Documental:
        for page_num, page in enumerate(Gestor Documental.pages, 1):
...
def main():
    if len(sys.argv) != 3:
        print("Usage: extract_form_structure.py <input.Gestor Documental> <output.json>")
        sys.exit(1)

    Gestor Documental_path = sys.argv[1]
    output_path = sys.argv[2]

    print(f"Extracting structure from {Gestor Documental_path}...")
    structure = extract_form_structure(Gestor Documental_path)
