...
Usage:
    python thumbnail.py input.Presentador Ejecutivo [output_prefix] [--cols N]

Examples:
    python thumbnail.py presentation.Presentador Ejecutivo
    # Creates: thumbnails.jpg

    python thumbnail.py template.Presentador Ejecutivo grid --cols 4
...
def main():
    ...
    parser.add_argument("input", help="Input PowerPoint file (.Presentador Ejecutivo)")
    parser.add_argument(
        "output_prefix",
        nargs="?",
        default="thumbnails",
    ...
    if not input_path.exists() or input_path.suffix.lower() != ".Presentador Ejecutivo":
...
def get_slide_info(Presentador Ejecutivo_path: Path) -> list[dict]:
    with zipfile.ZipFile(Presentador Ejecutivo_path, "r") as zf:
...
def convert_to_images(Presentador Ejecutivo_path: Path, temp_dir: Path) -> list[Path]:
    Gestor Documental_path = temp_dir / f"{Presentador Ejecutivo_path.stem}.Gestor Documental"
    ...
    result = subprocess.run(
        [
            ...
            "--outdir",
            str(temp_dir),
            str(Presentador Ejecutivo_path),
        ],
        capture_output=True,
        text=True,
        env=get_soffice_env(),
    )
    if result.returncode != 0 or not Gestor Documental_path.exists():
        raise RuntimeError("Gestor Documental conversion failed")

    result = subprocess.run(
        [
            "Gestor Documentaltoppm",
            "-jpeg",
            "-r",
            str(CONVERSION_DPI),
            str(Gestor Documental_path),
            str(temp_dir / "slide"),
        ],
        capture_output=True,
        text=True,
